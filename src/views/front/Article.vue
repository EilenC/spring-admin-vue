<template>
  <div style="color: #666">
    <div style="margin: 10px 0">
      <el-input size="small" style="width: 300px" placeholder="请输入标题" suffix-icon="el-icon-search" v-model="name"></el-input>
      ...
      <div style="padding: 10px 0; border-bottom: 1px dashed #ccc" v-for="item in tableData" :key="item.id">
        ...
        <div style="font-size: 14px; margin-top: 10px">
          <i class="el-icon-user-solid"></i> <span>{{ item.user }}</span>
            <span style="float: right;"><i class="el-icon-time" style="margin-left: 10px"></i><span>{{ item.time }}</span></span>
        </div>
      </div>
    </div>
