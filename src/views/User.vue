<template>
    <div>
        ...
        <div style="margin: 10px 0">
            ...
            <el-upload action="http://localhost:9090/user/input" :show-file-list="false" accept="xlsx"
...
export default {
    ...
    methods: {
        load() {
            ...
            this.request.get("/role/allRole", {}).then(res => {
                if (res.code === '200') {
        ...
        handleSelectionChange(val) {
            this.selectAllID = val
        },
        delBatch() {
            let ids = this.selectAllID.map(v => v.id)
            this.request.delete("/user/batch", {
                data: ids,
            }).then(res => {
                if (res.code === '200') {
